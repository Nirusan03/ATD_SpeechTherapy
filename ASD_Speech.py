...
def check_pronunciation(spoken_word):
    if spoken_word in target_words:
        ...
        if not correct_phonemes:
            return f"Pronunciation data not available for the word '{spoken_word}'."
        
        # Providing a simple correct pronunciation example
...
while True:
    ...
    if recognizer.AcceptWaveform(data):
        ...
        # Print the recognized text to check if it's working
        print(f"You said: {text}")

        if text:
            if text in target_words:
                feedback = check_pronunciation(text)
                provide_feedback(feedback)
            else:
                provide_feedback(f"'{text}' is either mispronounced or not in the list. Try again.")
        else:
